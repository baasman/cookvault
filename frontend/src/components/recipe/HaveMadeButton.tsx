...
const HaveMadeButton: React.FC<HaveMadeButtonProps> = ({
  recipe,
  size = 'sm',
  iconOnly = false,
  onSuccess
  ...
  const toggleMutation = useMutation({
    mutationFn: () => recipeGroupsApi.toggleSystemGroup('have_made', recipe.id),
    onSuccess: (data) => {
  ...
  const haveMade = recipe.have_made || false;
  ...
  const getIcon = () => {
    ...
    return (
      <svg
        className={`h-4 w-4 ${haveMade ? 'text-green-600' : 'text-gray-500'}`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
  ...
  const getButtonText = () => {
    ...
    return haveMade ? 'Have Made' : 'Have Made';
  };
  ...
  return (
    <button
      ...
      title={haveMade ? 'Remove from Have Made' : 'Mark as Have Made'}
      className={`
