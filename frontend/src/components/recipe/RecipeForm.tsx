...
export const RecipeForm: React.FC<RecipeFormProps> = ({
  ...
  const handleSaveMetadata = async () => {
    try {
      ...
      if (mode === 'create') {
        // For create mode, we need to create the recipe first if it doesn't exist
        if (!recipe) {
          throw new Error('Recipe not found in create mode');
        }
        // Update the existing empty recipe
        const updatedRecipe = await recipesApi.updateRecipe(recipe.id, formData);
        onSave(updatedRecipe);
      } else {
        // Edit mode
        if (!recipe) {
          throw new Error('Recipe not found in edit mode');
        }
        const updatedRecipe = await recipesApi.updateRecipe(recipe.id, formData);
        onSave(updatedRecipe);
      }
