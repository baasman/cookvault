  ...
  return (
    <Link to={`/cookbooks/${cookbook.id}`} onClick={handleClick} className="h-full">
      <div className="group bg-white rounded-xl shadow-sm border transition-all duration-200 hover:shadow-md hover:border-accent/20 overflow-hidden h-full flex flex-col" style={{borderColor: '#e8d7cf'}}>
        {/* Cookbook Cover */}
        <div className="aspect-[3/4] bg-gradient-to-br from-background-secondary to-primary-200 relative overflow-hidden flex-shrink-0">
          {/* Ownership indicator */}
          {isOwnCookbook && (
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          <div className="flex-grow">
            ...
            {cookbook.description && (
              <p className="text-xs text-text-secondary line-clamp-3 mb-3 description-text-compact">
            ...
            <div className="text-xs text-text-secondary space-y-1">
              {cookbook.publisher && (
                <div className="flex items-center">
                  ...
                  <span className="truncate">{cookbook.publisher}</span>
                </div>
              )}
              ...
              {cookbook.isbn && (
                <div className="flex items-center">
                  ...
                  <span className="truncate">ISBN: {cookbook.isbn}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-auto">
            ...
            {/* Uploaded by info - only show if user_id exists and it's not own cookbook */}
            {cookbook.user_id && !isOwnCookbook && (
              <div className={`text-xs text-text-secondary ${showPurchaseButton && cookbook.is_purchasable ? 'mt-2' : 'mt-3 pt-3 border-t border-gray-100'}`}>
                <div className="flex items-center">
                  <svg className="h-3 w-3 mr-1 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <Link
                    to={`/users/${cookbook.user_id}`}
                    onClick={(e) => {
                      e.stopPropagation();
                    }}
                    className="font-medium text-blue-600 hover:text-blue-800 hover:underline transition-colors cursor-pointer ml-1"
                    style={{color: '#2563eb'}}
                  >
                    User #{cookbook.user_id}
                  </Link>
                </div>
              </div>
            )}
