...
class PrintMarksRenderer:
    ...
    def render_page_info(
        self, canvas: Canvas, dimensions: PrintDimensions, page_info: Dict[str, Any]
    ):
    ...
    def get_content_frame_bounds(
        cls, dimensions: PrintDimensions, gutter_margin: float = 0
    ) -> Dict[str, float]:
        """
    ...
    def validate_content_placement(
        ...
        if (
            x < safe_bounds["left"]
            or y < safe_bounds["bottom"]
            or x + width > safe_bounds["right"]
            or y + height > safe_bounds["top"]
        ):
        ...
        if (
            x < trim_bounds["left"]
            or y < trim_bounds["bottom"]
            or x + width > trim_bounds["right"]
            or y + height > trim_bounds["top"]
        ):
        ...
        if (
            x < 0
            or y < 0
            or x + width > dimensions.full_width
            or y + height > dimensions.full_height
        ):
            result["in_bleed_area"] = False
            result["is_valid"] = False
            result["warnings"].append(
                "Content extends beyond bleed area - will be cut off"
            )
