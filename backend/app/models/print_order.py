...
from sqlalchemy import String, Integer, DateTime, ForeignKey, Boolean, Numeric, Text, JSON
...
class PrintSpecification(db.Model):
    ...
    lulu_page_count: Mapped[Optional[int]] = mapped_column(Integer)  # May differ from our count
...
class PrintOrder(db.Model):
    ...
    shipping_country_code: Mapped[str] = mapped_column(String(2), default="US", nullable=False)
    ...
    submitted_at: Mapped[Optional[datetime]] = mapped_column(DateTime)  # When sent to Lulu
    paid_at: Mapped[Optional[datetime]] = mapped_column(DateTime)  # When payment completed
    ...
    cookbook: Mapped["Cookbook"] = relationship("Cookbook", back_populates="print_orders")
    specification: Mapped["PrintSpecification"] = relationship(
        "PrintSpecification", back_populates="print_orders"
    )
    payment: Mapped[Optional["Payment"]] = relationship("Payment", back_populates="print_order", foreign_keys="Payment.print_order_id")
    status_updates: Mapped[list["PrintOrderStatusUpdate"]] = relationship(
        "PrintOrderStatusUpdate", back_populates="print_order", cascade="all, delete-orphan"
    )
...
class PrintOrderStatusUpdate(db.Model):
    ...
    lulu_event_type: Mapped[Optional[str]] = mapped_column(String(100))  # Webhook event type
