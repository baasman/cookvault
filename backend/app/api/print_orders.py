...
from app.models.print_order import (
    PrintOrder, PrintSpecification, PrintOrderStatus, 
    TrimSize, BindingType, PaperType, CoverFinish
)
...
def validate_cover_data(current_user):
    ...
    try:
        ...
        cover_service = CoverGenerationService()
        validation_result = cover_service.validate_cover_content(cookbook_dict, trim_size)
...
def get_print_quote(current_user):
    ...
    try:
        ...
        spec_required = ['trim_size', 'binding_type', 'paper_type', 'cover_finish', 'page_count']
...
def create_print_order(current_user):
    ...
    try:
        ...
        # Validate request data
        required_fields = [
            'cookbook_id', 'quantity', 'specification', 'shipping_address'
        ]
        data = validate_json_data(request, required_fields)
        if isinstance(data, tuple):  # Error response
            return data
        ...
        shipping_required = [
            'name', 'address_line1', 'city', 'state_code', 'postal_code', 'country_code', 'email'
        ]
...
def create_print_order_payment(current_user, order_id):
    ...
    try:
        ...
        if order.status != PrintOrderStatus.DRAFT:
            return jsonify({'error': 'Payment can only be created for draft orders'}), 400

        # Create Stripe payment intent
        stripe_service = StripeService()
        payment_intent_data = stripe_service.create_print_order_payment_intent(current_user, order)
        ...
        db.session.rollback()
        logger.error(f"Error creating payment intent for print order {order_id}: {str(e)}")
...
def submit_print_order(current_user, order_id):
    ...
    try:
        ...
        if order.status != PrintOrderStatus.PAID:
            return jsonify({'error': 'Order can only be submitted after payment is completed'}), 400
        ...
        recipes_dict = [recipe.to_dict(current_user_id=current_user.id, is_admin=True) for recipe in recipes]
        ...
        try:
            ...
            interior_url = lulu_service.upload_interior_pdf(interior_pdf, interior_filename)
            ...
            logger.info(f"Generated and uploaded print-ready interior PDF for order {order.order_number}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Generated and uploaded print-ready cover PDF for order {order.order_number}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Successfully created Lulu print job {order.lulu_print_job_id} for order {order.order_number}")
        ...
        logger.info(f"Submitted print order {order.order_number} (Lulu job: {order.lulu_print_job_id})")
...
def check_refund_eligibility(current_user, order_id):
    ...
    try:
        ...
        logger.error(f"Error checking refund eligibility for order {order_id}: {str(e)}")
...
def create_manual_refund(current_user, order_id):
    ...
    try:
        ...
        try:
            ...
            amount_text = f"${refund_amount:.2f}" if refund_amount else f"${order.total_cost:.2f}"
            status_update = PrintOrderStatusUpdate(
                print_order_id=order.id,
            ...
            logger.info(f"Manual refund processed for order {order.order_number}: {refund_info.get('refund_id')}")
            ...
            logger.error(f"Failed to process manual refund for order {order.order_number}: {str(e)}")
    ...
    try:
        ...
        if refund_eligibility['eligible']:
            try:
                refund_info = stripe_service.create_print_order_refund(
                    ...
                    partial_amount=Decimal(str(refund_eligibility['refund_amount'])) if refund_eligibility['refund_percentage'] < 100 else None
                )
            except Exception as e:
        ...
        if order.lulu_print_job_id:
            ...
            if not cancel_result:
                logger.warning(f"Failed to cancel print job {order.lulu_print_job_id} with Lulu")
                lulu_cancel_success = False

        # Update order status (may have been updated by refund processing)
        previous_status = order.status
        final_status = PrintOrderStatus.REFUNDED if (refund_info and 'refund_id' in refund_info) else PrintOrderStatus.CANCELLED
        ...
        logger.info(f"Cancelled print order {order.order_number} - Status: {final_status.value}")
...
def reorder_print_order(current_user, order_id):
    ...
    try:
        ...
        original_order = PrintOrder.query.filter_by(id=order_id, user_id=current_user.id).first()
        ...
        logger.info(f"Created reorder {order_number} based on {original_order.order_number}")
