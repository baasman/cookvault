...
def create_subscription_upgrade():
    ...
    try:
        ...
        logger.error(
            f"Failed to create subscription upgrade for user {user_id}: {str(e)}"
        )
        import traceback
...
def create_cookbook_purchase():
    ...
    try:
        ...
        stripe_service = StripeService()
        payment_intent_data = stripe_service.create_cookbook_payment_intent(
            user, cookbook
        )

        return jsonify({"success": True, "payment_intent": payment_intent_data}), 200

    except Exception as e:
        logger.error(
            f"Failed to create cookbook purchase for user {user_id}, cookbook {cookbook_id}: {str(e)}"
        )
...
def confirm_subscription():
    ...
    try:
        ...
        stripe_sub = stripe.Subscription.retrieve(
            subscription.stripe_subscription_id, expand=["latest_invoice"]
        )
        ...
        recent_payments = (
            Payment.query.filter_by(
                user_id=user_id, payment_type=PaymentType.SUBSCRIPTION
            )
            ...
            if payment.stripe_payment_intent_id:
                try:
                    ...
                    if pi.status == "succeeded" and pi.payment_method:
                        ...
                        logger.info(
                            f"Found successful payment method: {payment_method_id}"
                        )
                        break
        ...
        if payment_method_id:
            ...
            logger.info("Set default payment method for customer")
            ...
            try:
                ...
                if paid_invoice.status == "paid":
                    ...
                    db.session.commit()
                    logger.info(
                        f"User {user_id} subscription activated after invoice payment"
                    )
...
def cancel_subscription():
    ...
    try:
        ...
        if success:
            return jsonify(
                {"success": True, "message": "Subscription canceled successfully"}
            ), 200
        else:
...
def get_user_subscription():
    ...
    try:
        ...
        return jsonify({"success": True, "subscription": sub_dict}), 200
...
def get_user_payment_methods():
    ...
    try:
        ...
        return jsonify({"success": True, "payment_methods": payment_methods}), 200
...
def get_payment_status():
    ...
    try:
        ...
        payment = Payment.query.filter_by(
            stripe_payment_intent_id=payment_intent_id, user_id=user_id
        ).first()
