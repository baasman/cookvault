...
def handle_lulu_status_webhook():
    ...
    try:
        ...
        signature = request.headers.get('X-Hub-Signature-256') or request.headers.get('X-Lulu-Signature')
        ...
        print_order = PrintOrder.query.filter_by(lulu_print_job_id=str(print_job_id)).first()
        if not print_order:
        ...
        if event_type in ['print_job.validation.completed', 'validation_completed']:
            ...
            validation_result = print_job_data.get('validation_result') or print_job_data.get('status')
            ...
            if shipping_info:
                ...
                if estimated_delivery:
                    try:
                        ...
                        logger.warning(f"Invalid delivery date format: {estimated_delivery}")
            ...
            failure_reason = print_job_data.get('failure_reason') or print_job_data.get('error_message')
            if failure_reason:
        ...
        if status_changed:
            ...
            logger.info(f"Print order {print_order.order_number} status changed: {previous_status.value} -> {print_order.status.value}")
...
def handle_lulu_validation_webhook():
    ...
    try:
        ...
        if status_changed:
            ...
            logger.info(f"Print order {print_order.order_number} validation status updated")
