...
class PDFStyleManager:
        ...
        try:
            ...
            if (fonts_dir / "Inter-Regular.ttf").exists():
                pdfmetrics.registerFont(
                    TTFont("Inter", str(fonts_dir / "Inter-Regular.ttf"))
                )
                pdfmetrics.registerFont(
                    TTFont("Inter-Medium", str(fonts_dir / "Inter-Medium.ttf"))
                )
                pdfmetrics.registerFont(
                    TTFont("Inter-SemiBold", str(fonts_dir / "Inter-SemiBold.ttf"))
                )
                pdfmetrics.registerFont(
                    TTFont("Inter-Bold", str(fonts_dir / "Inter-Bold.ttf"))
                )
            ...
            if (fonts_dir / "Lora-Regular.ttf").exists():
                pdfmetrics.registerFont(
                    TTFont("Lora", str(fonts_dir / "Lora-Regular.ttf"))
                )
                pdfmetrics.registerFont(
                    TTFont("Lora-Bold", str(fonts_dir / "Lora-Bold.ttf"))
                )
            ...
            if (fonts_dir / "JetBrainsMono-Regular.ttf").exists():
                pdfmetrics.registerFont(
                    TTFont(
                        "JetBrainsMono", str(fonts_dir / "JetBrainsMono-Regular.ttf")
                    )
                )
                pdfmetrics.registerFont(
                    TTFont(
                        "JetBrainsMono-Medium",
                        str(fonts_dir / "JetBrainsMono-Medium.ttf"),
                    )
                )
    ...
    def _apply_modern_styles(self):
        ...
        "Lora" if self.fonts_available.get("lora") else "Helvetica"
        serif_bold = (
            "Lora-Bold" if self.fonts_available.get("lora") else "Helvetica-Bold"
        )
        sans_font = "Inter" if self.fonts_available.get("inter") else "Helvetica"
        "Inter-Medium" if self.fonts_available.get("inter") else "Helvetica-Bold"
        "Inter-SemiBold" if self.fonts_available.get("inter") else "Helvetica-Bold"
        sans_bold = (
            "Inter-Bold" if self.fonts_available.get("inter") else "Helvetica-Bold"
        )
        mono_font = (
            "JetBrainsMono" if self.fonts_available.get("jetbrains") else "Courier"
        )
    ...
    def get_color_for_profile(
        self, color_rgb: tuple[int, int, int], profile: str = "digital"
    ) -> colors.Color:
        """
    ...
    def create_pull_quote(
        self, quote_text: str, author: str = None, style_name: str = None
    ):
        """
        ...
        if style_name is None:
            ...
            if "NoteText" in self.styles:
                ...
                quote_style = ParagraphStyle(
                    ...
                    parent=self.styles["Normal"],
                    fontSize=11,
                    textColor=self.modern_colors.get(
                        "secondary", colors.HexColor("#8D99AE")
                    ),
                    leftIndent=24,
                    rightIndent=24,
                    spaceBefore=12,
    ...
    def _apply_book_styles(self):
        ...
        serif_font = (
            "CormorantGaramond"
            if "CormorantGaramond" in registered_fonts
            else "Times-Roman"
        )
        serif_bold = (
            "CormorantGaramond-Bold"
            if "CormorantGaramond-Bold" in registered_fonts
            else "Times-Bold"
        )
        serif_italic = (
            "CormorantGaramond-Italic"
            if "CormorantGaramond-Italic" in registered_fonts
            else "Times-Italic"
        )
        sans_font = "Inter" if "Inter" in registered_fonts else "Helvetica"
        sans_bold = (
            "Inter-Medium" if "Inter-Medium" in registered_fonts else "Helvetica-Bold"
        )
...
class PDFImageHandler:
        ...
        try:
            ...
            with PILImage.open(source_path) as img:
                ...
                if self.config.color_mode == "CMYK" and img.mode != "CMYK":
                    ...
                    logger.debug(
                        f"Converted image to CMYK for {self.config.profile.value} profile"
                    )
                ...
                logger.debug(
                    f"Optimized image: quality={self.config.image_quality}, mode={self.config.color_mode}"
                )
    ...
    def process_image(
        ...
        apply_profile_optimization: bool = True,
    ) -> Optional[RLImage]:
        """
    ...
    def get_cover_image(
        self, url: str, page_width: float, page_height: float, cover_fit: str = "cover"
    ) -> Optional[RLImage]:
        """
        ...
        try:
            ...
            if url in self._image_cache:
                ...
                return self._process_cover_from_path(
                    cached_path, page_width, page_height, cover_fit
                )
            ...
            img = self._process_cover_from_path(
                tmp_path, page_width, page_height, cover_fit
            )
    ...
    def _process_cover_from_path(
        self, image_path: str, page_width: float, page_height: float, cover_fit: str
    ) -> Optional[RLImage]:
    ...
    def get_image_from_url(
        self, url: str, max_width: float = 4 * inch, max_height: float = 3 * inch
    ) -> Optional[RLImage]:
        """
...
class RecipePDFBuilder:
    ...
    def build_recipe_pdf(self, recipe: Dict[str, Any]) -> bytes:
        ...
        if metadata_parts:
            ...
            story.append(
                Paragraph(
                    " • ".join(metadata_parts), self.style_manager.styles["Metadata"]
                )
            )
        ...
        if self.config.include_images and recipe.get("images"):
            ...
            if main_image.get("cloudinary_url"):
                img = self.image_handler.get_image_from_url(
                    main_image["cloudinary_url"], max_width, max_height
                )
            elif main_image.get("image_url"):
                img = self.image_handler.get_image_from_url(
                    main_image["image_url"], max_width, max_height
                )
            ...
            if img:
                ...
                if self.config.template == PDFTemplate.MODERN:
                    ...
                    image_table = Table(
                        [[img]], colWidths=[img.drawWidth], rowHeights=[img.drawHeight]
                    )
                        ...
                        TableStyle(
                            [
                                ...
                                (
                                    ...
                                    (-1, -1),
                                    0.5,
                                    self.style_manager.modern_colors.get(
                                        "divider", colors.HexColor("#EDF2F4")
                                    ),
                                ),
                    ...
                    if caption:
                        ...
                        story.append(
                            Paragraph(
                                caption,
                                self.style_manager.styles.get(
                                    "ImageCaption", self.style_manager.styles["Normal"]
                                ),
                            )
                        )
        ...
        story.append(
            Paragraph("Ingredients", self.style_manager.styles["SectionHeader"])
        )
        story.append(Spacer(1, 0.1 * inch))
        ...
        story.append(
            Paragraph("Instructions", self.style_manager.styles["SectionHeader"])
        )
        story.append(Spacer(1, 0.1 * inch))
        ...
        for i, instruction in enumerate(instructions, 1):
            if isinstance(instruction, dict):
                ...
                inst_image_url = instruction.get("cloudinary_url") or instruction.get(
                    "image_url"
                )
            else:
            ...
            if (
                self.config.template == PDFTemplate.MODERN
                ...
                inst_img = self.image_handler.get_image_from_url(
                    inst_image_url, max_width=2 * inch, max_height=1.5 * inch
                )
                ...
                if inst_img:
                    ...
                    inst_table = Table(
                        ...
                        colWidths=[
                            inst_img.drawWidth + 12,
                            None,
                        ],  # Image width + padding, rest for text
                        spaceBefore=6,
                        spaceAfter=6,
                    )
                    inst_table.setStyle(
                ...
                story.append(
                    Paragraph(
                        f"{i}. {inst_text}", self.style_manager.styles["Instruction"]
                    )
                )
                story.append(Spacer(1, 0.1 * inch))
        ...
        if self.config.include_notes and recipe.get("notes"):
            ...
            story.append(Spacer(1, 0.1 * inch))
            story.append(
                Paragraph(recipe["notes"], self.style_manager.styles["Normal"])
            )
        ...
        def cleanup_element(element):
            if hasattr(element, "_temp_file_path"):
                try:
                    ...
                    logger.warning(
                        f"Failed to cleanup temp file {element._temp_file_path}: {e}"
                    )
...
class BookRecipePDFBuilder:
    ...
    def build_recipe_pdf(self, recipe: Dict[str, Any]) -> bytes:
        ...
        frame_width = (
            page_width - 1.5 * inch
        ) / 2 - 0.2 * inch  # Space for margins and column gap
        ...
        right_frame = Frame(
            0.75 * inch
            + frame_width
            + 0.2 * inch,  # x (left margin + left frame + gap)
            0.75 * inch,  # y
            frame_width,  # width
            frame_height,  # height
    ...
    def _build_recipe_story(self, recipe: Dict[str, Any]) -> List:
        ...
        if recipe.get("description"):
            ...
            story.append(
                Paragraph(desc_text, self.style_manager.styles["RecipeDescription"])
            )
        ...
        if metadata_parts:
            ...
            story.append(
                Paragraph(metadata_text, self.style_manager.styles["MetadataBar"])
            )
        ...
        for ingredient in ingredients:
            ...
            story.append(
                Paragraph(f"• {ing_text}", self.style_manager.styles["Ingredient"])
            )
        ...
        for i, instruction in enumerate(instructions, 1):
            ...
            story.append(
                Paragraph(f"{i}. {inst_text}", self.style_manager.styles["Instruction"])
            )
            story.append(Spacer(1, 4))
...
class CookbookPDFBuilder:
    ...
    def build_cookbook_pdf(
        self, cookbook: Dict[str, Any], recipes: List[Dict[str, Any]]
    ) -> bytes:
    ...
    def _build_cover_page(self, cookbook: Dict[str, Any]) -> List:
        ...
        if self.config.template == PDFTemplate.MODERN and cookbook.get(
            "cover_image_url"
        ):
        ...
        elements.append(Paragraph(cookbook.get("title", "Cookbook"), title_style))
        ...
        if cookbook.get("author"):
            ...
            elements.append(Paragraph(f"by {cookbook['author']}", author_style))
        ...
        if cookbook.get("description"):
            ...
            elements.append(
                Paragraph(
                    sanitize_html_for_reportlab(cookbook["description"]), desc_style
                )
            )
        ...
        if pub_info:
            ...
            elements.append(Paragraph(" • ".join(pub_info), pub_style))
    ...
    def _create_modern_cover(self, cookbook: Dict[str, Any]):
        ...
        class ModernCoverPage(Flowable):
            ...
            def __init__(
                self, cookbook, image_handler, style_manager, page_width, page_height
            ):
        ...
        return ModernCoverPage(
            cookbook, self.image_handler, self.style_manager, page_width, page_height
        )
    ...
    def _build_table_of_contents(self, recipes: List[Dict[str, Any]]) -> List:
        ...
        if self.config.template == PDFTemplate.MODERN:
            ...
            toc_title_style = ParagraphStyle(
                "ModernTOCTitle",
                parent=self.style_manager.styles.get(
                    "CookbookTitle", self.style_manager.styles["Heading1"]
                ),
                fontSize=36,
                alignment=TA_CENTER,
            ...
            toc_entry_style = ParagraphStyle(
                "ModernTOCEntry",
                parent=self.style_manager.styles.get(
                    "ModernBody", self.style_manager.styles["Normal"]
                ),
                fontSize=12,
                leftIndent=0,
                spaceBefore=8,
    ...
    def _build_section_divider(self, category_name: str) -> List:
        ...
        if self.config.template == PDFTemplate.MODERN:
            ...
            divider_style = ParagraphStyle(
                ...
                parent=self.style_manager.styles.get(
                    "RecipeTitle", self.style_manager.styles["Heading1"]
                ),
                fontSize=32,
                textColor=self.style_manager.modern_colors.get(
                    "primary", colors.HexColor("#2B2D42")
                ),
                alignment=TA_CENTER,
                spaceBefore=0,
                spaceAfter=24,
            ...
            line = HRFlowable(
                width="30%",
                thickness=0.5,
                color=self.style_manager.modern_colors.get(
                    "divider", colors.HexColor("#EDF2F4")
                ),
                spaceAfter=0,
                spaceBefore=12,
        ...
        for i, recipe in enumerate(recipes):
            for ingredient in recipe.get("ingredients", []):
                ...
                if ing_name:
                    ...
                    ingredient_map[ing_name].append(
                        recipe.get("title", f"Recipe {i + 1}")
                    )
...
class PDFService:
    ...
    def generate_recipe_pdf(
        self, recipe: Dict[str, Any], config: PDFConfig = None
    ) -> bytes:
        """Generate PDF for a single recipe"""
        ...
        # Use book-style builder for book template
            ...
            if config:
                ...
                self.book_recipe_builder.style_manager = PDFStyleManager(
                    config.template
                )
    ...
    def generate_cookbook_pdf(
        ...
        if config:
            ...
            self.cookbook_builder.recipe_builder.style_manager = PDFStyleManager(
                config.template
            )
    ...
    def generate_recipe_collection_pdf(
        self,
        recipes: List[Dict[str, Any]],
    ...
    def generate_simple_cover(
        self, cookbook: Dict[str, Any], config: PDFConfig = None
    ) -> bytes:
        ...
        doc = SimpleDocTemplate(
            buffer, pagesize=page_size, topMargin=1 * inch, bottomMargin=1 * inch
        )
