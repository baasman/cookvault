...
class LuluAPIError(Exception):
    ...
    def _determine_retryable(self) -> bool:
        ...
        # Retryable errors
        if self.status_code in [429, 502, 503, 504]:  # Rate limit, bad gateway, service unavailable
            return True
...
class LuluService:
    ...
    def _refresh_access_token(self, max_retries: int = 3) -> str:
        ...
        """
        auth_url = urljoin(self.api_url, "/auth/realms/glasstree/protocol/openid-connect/token")
        ...
        for attempt in range(max_retries):
            try:
                ...
                self._refresh_token = token_data.get("refresh_token")  # Store if available
                ...
                logger.info(f"Successfully refreshed Lulu API access token (expires in {expires_in}s)")
                return self._access_token
                ...
                last_error = LuluAPIError(f"Connection error during authentication: {str(e)}")
    ...
    def _make_request(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.error(f"Lulu API request timed out: {method} {endpoint} (attempt {attempt + 1})")
                if attempt < max_retries:
                    time.sleep(1)
                    continue
    ...
    def upload_pdf_file(
        ...
        logger.info(f"Uploading {file_type} PDF file: {filename} ({len(pdf_bytes)} bytes)")
    ...
    def upload_interior_pdf(self, pdf_bytes: bytes, filename: Optional[str] = None) -> Optional[str]:
        """
    ...
    def upload_cover_pdf(self, pdf_bytes: bytes, filename: Optional[str] = None) -> Optional[str]:
        """
    ...
    def validate_uploaded_file(
        ...
        endpoint = "/print-file-validations/" if file_type == 'interior' else "/cover-validations/"
        ...
        try:
            ...
            if warnings:
                warning_list = [warning.get("message", str(warning)) for warning in warnings]
    ...
    def calculate_print_cost(
        ...
        shipping_cost = Decimal(str(response.get("shipping_cost", {}).get("total_cost_incl_tax", 0)))
    ...
    def get_print_quote(
        ...
        if self.dev_mode:
            ...
            binding_cost = Decimal('2.00') if specification.binding_type != BindingType.SADDLE_STITCH else Decimal('0.50')
            color_cost = Decimal('3.00') if specification.color_pages else Decimal('0.00')
            printing_cost = (base_cost + page_cost + binding_cost + color_cost) * quantity
        ...
        try:
            ...
            estimated_delivery_days = self._estimate_delivery_days(shipping_country, shipping_state)
            ...
            logger.info(f"Generated print quote {quote_id}: ${costs['total_cost']:.2f} for {quantity} copies")
            return quote
    ...
    def _estimate_delivery_days(self, shipping_country: str, shipping_state: Optional[str] = None) -> int:
        """
    ...
    def _get_dynamic_markup_rate(self, quantity: int) -> Decimal:
        ...
        """
        base_markup = Decimal(str(current_app.config.get('PRINT_PLATFORM_MARKUP', 0.25)))
    ...
    def validate_pdf_file(
        ...
        """
        endpoint = "/printfile-validation/" if not is_cover else "/coverfile-validation/"
    ...
    def create_print_job(
        ...
        try:
            ...
            if not interior_file_url:
                raise LuluAPIError("Interior file URL not found - file must be uploaded first")
            if not cover_file_url:
                raise LuluAPIError("Cover file URL not found - file must be uploaded first")
            
            # Validate files before creating job
            logger.info("Validating uploaded files")
            
            # Validate interior file
            interior_valid, interior_error, interior_details = self.validate_uploaded_file(
                interior_file_url, 
                'interior', 
            ...
            line_item = {
                "title": f"Cookbook: {order.cookbook.title}" if order.cookbook else f"Cookbook Order {order.order_number}",
            ...
            if order.cookbook:
                line_item.update({
                    ...
                    "description": order.cookbook.description[:500] if order.cookbook.description else None
                })
            ...
            logger.info(f"Submitting print job to Lulu API with {order.quantity} copies")
            ...
            logger.error(f"Unexpected error creating print job: {str(e)}", exc_info=True)
    ...
    def get_multiple_job_statuses(self, print_job_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            response = self._make_request(
                "POST", 
                "/print-jobs/batch-status/",
                data={"job_ids": print_job_ids}
            )
            ...
            return self.get_multiple_job_statuses(print_job_ids[:5])  # Recursive with limit
    ...
    def cancel_print_job(self, print_job_id: str, reason: Optional[str] = None) -> bool:
        ...
        try:
            ...
            response = self._make_request(
                "POST", 
                f"/print-jobs/{print_job_id}/actions/",
                data=request_data
            )
    ...
    def handle_api_error(self, error: Exception, context: str = "") -> None:
        ...
        if isinstance(error, LuluAuthenticationError):
            logger.error(f"Lulu authentication failed{error_context}. Check API credentials.")
        elif isinstance(error, LuluValidationError):
            logger.error(f"Lulu file validation failed{error_context}. PDF may not meet requirements.")
            ...
            logger.error(f"Lulu API quota exceeded{error_context}. Consider upgrading plan or reducing usage.")
            ...
            if error.is_retryable:
                ...
                logger.warning(f"Retryable Lulu API error{error_context}. Retry in {delay}s. Error: {error}")
            else:
