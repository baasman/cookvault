...
class PrintReadyBaseDocTemplate(BaseDocTemplate):
    ...
    def handle_pageBegin(self):
        ...
        self.print_marks_renderer.create_print_ready_template(
            self.canv, self.dimensions, page_info=page_info
        )
...
class PrintReadyPDFBuilder:
    ...
    def __init__(self, config: PDFConfig):
        if not config.print_ready or not config.trim_size:
            raise ValueError(
                "PrintReadyPDFBuilder requires print_ready=True and trim_size"
            )
        ...
        logger.info(
            f'Full size: {self.dimensions.full_width / inch:.2f}" x {self.dimensions.full_height / inch:.2f}"'
        )
        logger.info(
            f'Trim size: {self.dimensions.trim_width / inch:.2f}" x {self.dimensions.trim_height / inch:.2f}"'
        )
    ...
    def build_cookbook_pdf(
        ...
        frame_bounds = PrintMarksRenderer.get_content_frame_bounds(
            self.dimensions, gutter_margin=gutter_margin
        )
        ...
        page_template = PageTemplate(
            id="cookbook_page", frames=[content_frame], onPage=self._on_page_callback
        )
    ...
    def build_recipe_pdf(self, recipe: Dict[str, Any]) -> bytes:
        ...
        page_template = PageTemplate(
            id="recipe_page", frames=[content_frame], onPage=self._on_page_callback
        )
    ...
    def validate_content_safety(self, content_elements: List) -> Dict[str, Any]:
        ...
        return {"is_safe": True, "warnings": [], "recommendations": []}
