...
class CloudinaryService:
    ...
    def is_enabled(self) -> bool:
        ...
        return current_app.config.get("USE_CLOUDINARY", False) and self._initialized
    ...
    def upload_pdf(
        ...
        """Upload a PDF as a raw resource. Cloudinary serves raw resources
        at a stable URL once uploaded — same shape as upload_image but no
        transformation pipeline since PDFs aren't images."""
        if not self.is_enabled():
            raise RuntimeError("Cloudinary service is not enabled or configured")
    ...
    def delete_image(self, public_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Failed to delete image from Cloudinary: {public_id}, result: {result}"
                )
    ...
    def optimize_image_for_upload(
        self, image_data: bytes, max_size_mb: int = 8
    ) -> bytes:
        """
