...
class GoogleBooksMetadataExtractor:
    ...
    def extract_metadata_from_pdf_info(
        self, pdf_path: str, pdf_metadata: Dict = None
    ) -> Dict:
        """
        ...
        title_candidates, author_candidates = self._extract_info_from_filename(
            pdf_path_obj.name
        )
        ...
        book_info = self._search_google_books_multiple_strategies(
            title_candidates, author_candidates
        )

        if book_info:
            self.logger.info(
                f"📚 Found Google Books match: '{book_info['title']}' by {book_info['authors']}"
            )
            return book_info
        else:
            self.logger.warning("📚 No Google Books match found, using extracted info")
            return self._create_fallback_metadata(
                title_candidates, author_candidates, pdf_path_obj
            )
    ...
    def _extract_info_from_filename(self, filename: str) -> Tuple[List[str], List[str]]:
        ...
        by_match = re.search(
            r"(.+?)\s+by\s+(.+?)(?:\s*\(\d+\))?$", cleaned_name, re.IGNORECASE
        )
        if by_match:
        ...
        self.logger.debug(
            f"Extracted from filename - Titles: {title_candidates}, Authors: {author_candidates}"
        )
    ...
    def _clean_filename(self, filename: str) -> str:
        ...
        edition_pattern = (
            r"\b(?:\d+(?:st|nd|rd|th)\s+)?(?:edition|revised|updated|expanded)\b"
        )
        cleaned = re.sub(edition_pattern, "", cleaned, flags=re.IGNORECASE)

        return cleaned.strip()

    def _search_google_books_multiple_strategies(
        self, titles: List[str], authors: List[str]
    ) -> Optional[Dict]:
        ...
        for title in titles[:3]:  # Try first 3 title candidates
            for author in authors[:2]:  # Try first 2 author candidates
                if title and author:
                    result = self._search_google_books(
        ...
        for title in titles[:2]:
            if title:
                result = self._search_google_books(
                    f'"{title}" subject:cooking OR subject:cookbook'
                )
                if result:
                    return result
        ...
        for author in authors[:2]:
            if author and len(author) > 3:
                result = self._search_google_books(
                    f'inauthor:"{author}" subject:cooking'
                )
                if result:
                    return result
    ...
    def _select_best_match(
        self, items: List[Dict], original_query: str
    ) -> Optional[Dict]:
        ...
        for item in items:
            ...
            for category in categories:
                ...
                if any(
                    word in category_lower for word in ["cooking", "food", "cookbook"]
                ):
                    score += 5
    ...
    def _create_fallback_metadata(
        self, titles: List[str], authors: List[str], pdf_path: Path
    ) -> Dict:
