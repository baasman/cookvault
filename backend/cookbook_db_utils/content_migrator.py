...
class ContentMigrator:
    ...
    def export_all_content(
        self, output_path: Optional[str] = None, include_user_data: bool = False
    ) -> bool:
        """
        ...
        try:
            with self.app.app_context():
                ...
                admin_user = self._get_or_create_admin_user(
                    admin_username, create_admin
                )
                if not admin_user:
                    return False

                print(
                    f"👤 Using admin user: {admin_user.username} (ID: {admin_user.id})"
                )
                ...
                try:
                    ...
                    success = True
                    success &= self._import_ingredients(
                        data.get("ingredients", []), dry_run
                    )
                    success &= self._import_cookbooks(
                        data.get("cookbooks", []), admin_user.id, dry_run
                    )
                    success &= self._import_recipes(
                        data.get("recipes", []), admin_user.id, dry_run
                    )
    ...
    def _collect_comprehensive_data(self, include_user_data: bool = False) -> Dict:
        ...
        data["ingredients"] = [
            ingredient.to_dict() for ingredient in Ingredient.query.all()
        ]
        ...
        data["instructions"] = [
            instruction.to_dict() for instruction in Instruction.query.all()
        ]
        ...
        if include_user_data:
            ...
            data["recipe_comments"] = [
                comment.to_dict() for comment in RecipeComment.query.all()
            ]
            total_records += len(data["recipe_comments"])

            print("   📑 Collecting user recipe collections...")
            data["user_recipe_collections"] = [
                collection.to_dict() for collection in UserRecipeCollection.query.all()
            ]
                ...
                if recipe_images_dir.exists():
                    for image_file in recipe_images_dir.glob("*"):
                        if image_file.is_file():
                            zipf.write(
                                image_file, f"images/recipe_images/{image_file.name}"
                            )
                            images_exported += 1
    ...
    def _display_import_metadata(self, metadata: Dict) -> None:
        ...
        print(
            f"   🌍 Source environment: {metadata.get('source_environment', 'Unknown')}"
        )
    ...
    def _get_or_create_admin_user(
        self, username: str, create_if_missing: bool
    ) -> Optional[User]:
        ...
        print("❌ No admin user found and create_admin=False")
        ...
        try:
            for ingredient_data in ingredients_data:
                ...
                if existing:
                    ...
                    print(
                        f"   ↩️  Ingredient '{ingredient_data['name']}' already exists, using existing"
                    )
                else:
                    ...
                    if not dry_run:
                        ...
                        db.session.flush()
                        self.id_mappings["ingredients"][ingredient_data["id"]] = (
                            ingredient.id
                        )
                    imported += 1
        ...
        try:
            for tag_data in tags_data:
                ...
                if existing:
                    ...
                    print(
                        f"   ↩️  Tag '{tag_data['name']}' already exists, using existing"
                    )
                else:
                    # Create new tag
                    if not dry_run:
    ...
    def _import_cookbooks(
        self, cookbooks_data: List[Dict], admin_user_id: int, dry_run: bool
    ) -> bool:
        ...
        try:
            for cookbook_data in cookbooks_data:
                ...
                existing = Cookbook.query.filter_by(
                    title=cookbook_data["title"], author=cookbook_data.get("author")
                ).first()
                ...
                if existing:
                    ...
                    print(
                        f"   ↩️  Cookbook '{cookbook_data['title']}' already exists, using existing"
                    )
                else:
                        ...
                        if cookbook_data.get("publication_date"):
                            try:
                                ...
                                pub_date = datetime.fromisoformat(
                                    cookbook_data["publication_date"].replace(
                                        "Z", "+00:00"
                                    )
                                )
                            except Exception:
    ...
    def _import_recipes(
        self, recipes_data: List[Dict], admin_user_id: int, dry_run: bool
    ) -> bool:
        ...
        try:
            for recipe_data in recipes_data:
                ...
                if recipe_data.get("cookbook") and recipe_data["cookbook"].get("id"):
                    cookbook_id = self.id_mappings["cookbooks"].get(
                        recipe_data["cookbook"]["id"]
                    )

                existing = None
                if cookbook_id:
                    existing = Recipe.query.filter_by(
                        title=recipe_data["title"], cookbook_id=cookbook_id
                    ).first()
                else:
                    existing = Recipe.query.filter_by(
                ...
                if existing:
                    ...
                    print(
                        f"   ↩️  Recipe '{recipe_data['title']}' already exists, using existing"
                    )
                else:
                    ...
                    if not dry_run:
                        recipe = Recipe(
                            ...
                            user_id=admin_user_id,  # Assign to admin user
                            is_public=recipe_data.get(
                                "is_public", False
                            ),  # Preserve original privacy status
                        )
                        ...
                        self._import_recipe_instructions(
                            recipe, recipe_data.get("instructions", [])
                        )
                        ...
                        self._import_recipe_images(
                            recipe, recipe_data.get("images", [])
                        )
            ...
            print(
                f"   ✅ Imported {imported} new recipes (assigned to admin, privacy status preserved)"
            )
            return True
    ...
    def _import_recipe_instructions(
        self, recipe: Recipe, instructions_data: List[Dict]
    ) -> None:
        ...
        for instruction_data in instructions_data:
            ...
            self.import_stats["instructions_imported"] = (
                self.import_stats.get("instructions_imported", 0) + 1
            )

    def _import_recipe_tags(self, recipe: Recipe, tags_data: List[Dict]) -> None:
        """Import tags for a recipe"""
        for tag_data in tags_data:
            tag = Tag(recipe_id=recipe.id, name=tag_data.get("name"))
            db.session.add(tag)
            self.import_stats["tags_imported"] = (
                self.import_stats.get("tags_imported", 0) + 1
            )
    ...
    def _import_recipe_images(self, recipe: Recipe, images_data: List[Dict]) -> None:
        ...
        for image_data in images_data:
            ...
            self.import_stats["images_imported"] = (
                self.import_stats.get("images_imported", 0) + 1
            )
            ...
            try:
                ...
                with zipfile.ZipFile(input_path, "r") as zipf:
                    ...
                    if images_dir.exists():
                        print(
                            f"📸 Found images directory with {len(list(images_dir.rglob('*')))} files"
                        )
    ...
    def _display_import_statistics(self) -> None:
        ...
        print("\n📊 Import Statistics:")
        print(
            f"   📚 Cookbooks imported: {self.import_stats.get('cookbooks_imported', 0)}"
        )
        print(f"   🍽️  Recipes imported: {self.import_stats.get('recipes_imported', 0)}")
        print(
            f"   🥕 Ingredients imported: {self.import_stats.get('ingredients_imported', 0)}"
        )
        print(f"   🏷️  Tags imported: {self.import_stats.get('tags_imported', 0)}")
        print(
            f"   📝 Instructions imported: {self.import_stats.get('instructions_imported', 0)}"
        )
        print(f"   📸 Images imported: {self.import_stats.get('images_imported', 0)}")
        print(
            f"   🗂️  Image files copied: {self.import_stats.get('image_files_copied', 0)}"
        )
