...
class DatabaseUtils:
    ...
    def import_data(
        self, input_path: str, format: str = "json", merge_strategy: str = "skip"
    ) -> bool:
                ...
                if stats["tables"].get("recipes", 0) > 0:
                    ...
                    if recipes:
                        ...
                        stats["recipe_stats"] = {
                            ...
                            "avg_cook_time": sum(cook_times) / len(cook_times)
                            if cook_times
                            else 0,
        ...
        try:
            with self.app.app_context():
                ...
                orphaned_instructions = (
                    ...
                    .filter(~Instruction.recipe_id.in_(db.session.query(Recipe.id)))
                    .all()
                )
            ...
            if "ingredients" in data:
                ...
                for ingredient_data in data["ingredients"]:
                    ...
                    if existing and merge_strategy == "skip":
                        ...
                        existing.category = ingredient_data.get(
    ...
    def import_to_admin(
        ...
        try:
            ...
            migrator = ContentMigrator(self.config_name)
            return migrator.import_to_admin(
                input_path, admin_username, create_admin, dry_run
            )
        except Exception as e:
