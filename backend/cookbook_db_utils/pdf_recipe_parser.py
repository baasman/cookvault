...
class PDFRecipeParser:
            ...
            if title_confidence > 0.7 and current_recipe["title"]:
                ...
                if current_recipe["title"] and current_recipe["content_lines"]:
                    current_recipe["content"] = "\n".join(
                        current_recipe["content_lines"]
                    ).strip()
                    current_recipe["end_line"] = line_num - 1
                    current_recipe["line_count"] = (
                        current_recipe["end_line"] - current_recipe["start_line"]
                    )
    ...
    def _assess_title_likelihood(
        ...
        common_word_ratio = sum(
            1 for word in common_instruction_words if word in line_lower.split()
        ) / len(line.split())
    ...
    def _validate_recipe_segment(self, recipe: Dict) -> bool:
        ...
        recipe_confidence = self._calculate_recipe_confidence(
            recipe.get("title", ""), content
        )
    ...
    def _enhance_recipe_segment(self, recipe: Dict) -> Dict:
        ...
        enhanced["estimated_difficulty"] = max(
            difficulty_score, key=difficulty_score.get
        )
        ...
        if times_found:
            enhanced["estimated_cook_time"] = max(
                times_found
            )  # Take the longest time mentioned
    ...
    def parse_pdf_recipe(self, recipe_segment: Dict) -> Dict:
        ...
        try:
            ...
            with app.app_context():
                ...
                self.logger.info(
                    f"🤖 Calling LLM to parse PDF recipe: '{recipe_title}...'"
                )

                # Use the existing parser with our preprocessed text
                parsed_recipe = parser.parse_recipe_text(
                    preprocessed_text, use_cache=False
                )

                # Log successful LLM parsing
                self.logger.info(
                    f"✅ LLM successfully parsed PDF recipe: '{recipe_title}...'"
                )
                ...
                parsed_recipe.update(
                    {
                        "cookbook_metadata": {
                            ...
                            "estimated_difficulty": recipe_segment.get(
                                "estimated_difficulty"
                            ),
                            "estimated_cook_time": recipe_segment.get(
                                "estimated_cook_time"
                            ),
                            "estimated_servings": recipe_segment.get(
                                "estimated_servings"
                            ),
            ...
            recipe_title = recipe_segment.get("title", "Unknown")[:50]
            self.logger.warning(
                f"❌ LLM parsing failed for recipe '{recipe_title}...': {e}"
            )
            self.logger.info(
                f"🔄 Falling back to manual parsing for recipe '{recipe_title}...'"
            )
            return self._fallback_parse(recipe_segment)
    ...
    def _preprocess_for_modern_parser(self, recipe_segment: Dict) -> str:
        ...
        if self.enable_historical_conversions:
            ...
            for old_ingredient, new_ingredient in self.ingredient_modernization.items():
                ...
                full_text = re.sub(
                    pattern, new_ingredient, full_text, flags=re.IGNORECASE
                )
    ...
    def _fallback_parse(self, recipe_segment: Dict) -> Dict:
        ...
        self.logger.info(
            f"🔧 Using manual fallback parsing for recipe: '{title[:50]}...'"
        )
        ...
        return {
            "title": title,
            "description": "Recipe from PDF cookbook",
...
def process_pdf_cookbook(
    pdf_path: str, app_context=None, enable_historical_conversions=True
) -> List[Dict]:
    """
    ...
    parser = PDFRecipeParser(
        app_context, enable_historical_conversions=enable_historical_conversions
    )
...
def process_historical_cookbook_pdf(pdf_path: str, app_context=None) -> List[Dict]:
    ...
    """
    return process_pdf_cookbook(
        pdf_path, app_context, enable_historical_conversions=True
    )
