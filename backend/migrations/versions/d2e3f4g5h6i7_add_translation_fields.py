...
def upgrade():
    ...
    if is_sqlite:
        ...
        with op.batch_alter_table("recipe", schema=None) as batch_op:
            ...
            batch_op.add_column(
                sa.Column("source_language_name", sa.String(50), nullable=True)
            )
            batch_op.add_column(
                sa.Column(
                    "is_translated", sa.Boolean(), nullable=True, server_default="0"
                )
            )
            batch_op.add_column(
        ...
        with op.batch_alter_table("processing_job", schema=None) as batch_op:
            ...
            batch_op.add_column(
                sa.Column("detected_language_name", sa.String(50), nullable=True)
            )
    else:
        # PostgreSQL: Use standard operations
        # Add to recipe table
        op.add_column(
            "recipe", sa.Column("source_language", sa.String(10), nullable=True)
        )
        op.add_column(
            "recipe", sa.Column("source_language_name", sa.String(50), nullable=True)
        )
        op.add_column(
            "recipe",
            sa.Column(
                "is_translated", sa.Boolean(), nullable=True, server_default="false"
            ),
        )
        op.add_column(
            "recipe", sa.Column("original_title", sa.String(200), nullable=True)
        )
        op.add_column(
            "recipe", sa.Column("original_description", sa.Text(), nullable=True)
        )

        # Add to instruction table
        op.add_column(
            "instruction", sa.Column("original_text", sa.Text(), nullable=True)
        )
