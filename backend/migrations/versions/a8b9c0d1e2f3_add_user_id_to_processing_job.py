    ...
    if is_sqlite:
        ...
        with op.batch_alter_table("processing_job", schema=None) as batch_op:
            ...
            batch_op.create_foreign_key(
                "fk_processing_job_user_id", "user", ["user_id"], ["id"]
            )
    else:
        # PostgreSQL: Use standard operations
        op.add_column(
            "processing_job", sa.Column("user_id", sa.Integer(), nullable=True)
        )
        op.create_index("ix_processing_job_user_id", "processing_job", ["user_id"])
        op.create_foreign_key(
            "fk_processing_job_user_id", "processing_job", "user", ["user_id"], ["id"]
        )
...
def downgrade():
    ...
    if is_sqlite:
        ...
        # PostgreSQL: Use standard operations
        op.drop_constraint(
            "fk_processing_job_user_id", "processing_job", type_="foreignkey"
        )
