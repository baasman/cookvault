...
class TestPDFServiceTemplateSelection:
    ...
    def test_different_templates_produce_different_builders(self):
        ...
        assert classic_service.cookbook_builder.style_manager.template == PDFTemplate.CLASSIC
        assert modern_service.cookbook_builder.style_manager.template == PDFTemplate.MODERN
...
class TestPDFGeneration:
    ...
    def test_generate_cookbook_pdf_classic(self, sample_cookbook, multiple_recipes):
        ...
        pdf_bytes = service.generate_cookbook_pdf(sample_cookbook, multiple_recipes, config)
...
class TestOutputProfiles:
    ...
    def test_home_print_profile_generates_pdf(self, sample_recipe):
        ...
        config = PDFConfig(
            template=PDFTemplate.MODERN,
            profile=OutputProfile.HOME_PRINT
        )
        service = PDFService(config)
...
class TestEdgeCases:
    ...
    def test_unicode_content(self, sample_recipe):
        ...
        config = PDFConfig(template=PDFTemplate.CLASSIC)  # Classic has better unicode support
        service = PDFService(config)
    ...
    def test_single_recipe_cookbook(self, sample_cookbook, sample_recipe):
        ...
        pdf_bytes = service.generate_cookbook_pdf(sample_cookbook, [sample_recipe], config)
        assert pdf_bytes is not None
        assert pdf_bytes[:4] == b'%PDF'
        ...
        recipe_templates = [
            {
                ...
                'instructions': [
                    ...
                    {'step_number': 2, 'text': 'Combine flour, baking soda and salt in a bowl.'},
                    {'step_number': 3, 'text': 'Beat butter, sugars, eggs and vanilla until creamy.'},
                    {'step_number': 4, 'text': 'Gradually blend in flour mixture.'},
                    {'step_number': 5, 'text': 'Stir in chocolate chips.'},
                    {'step_number': 6, 'text': 'Drop rounded tablespoons onto ungreased baking sheets.'},
                    {'step_number': 7, 'text': 'Bake for 9 to 11 minutes or until golden brown.'},
                ],
            ...
            },
            {
                ...
                'ingredients': [
                    ...
                    {'quantity': '1/4', 'unit': 'cup', 'name': 'fresh parsley, chopped'},
                    {'quantity': '1/2', 'unit': 'cup', 'name': 'Parmesan cheese, grated'},
                    {'quantity': '', 'unit': '', 'name': 'Salt and pepper to taste'},
                ],
                'instructions': [
                    {'step_number': 1, 'text': 'Cook pasta according to package directions.'},
                    {'step_number': 2, 'text': 'Melt butter in a large skillet over medium heat.'},
                    {'step_number': 3, 'text': 'Add garlic and cook until fragrant, about 1 minute.'},
                    {'step_number': 4, 'text': 'Drain pasta, reserving 1/2 cup pasta water.'},
                    {'step_number': 5, 'text': 'Toss pasta with garlic butter and parsley.'},
                    {'step_number': 6, 'text': 'Add pasta water as needed for consistency.'},
                    {'step_number': 7, 'text': 'Top with Parmesan and serve immediately.'},
                ],
                'notes': None,
            },
            {
                ...
                'instructions': [
                    ...
                    {'step_number': 2, 'text': 'Mix honey, soy sauce, mustard, and garlic.'},
                    {'step_number': 3, 'text': 'Place salmon on a lined baking sheet.'},
                    {'step_number': 4, 'text': 'Brush glaze over salmon fillets.'},
                    {'step_number': 5, 'text': 'Bake 12-15 minutes until fish flakes easily.'},
                ],
            ...
            },
            {
                ...
                'ingredients': [
                    ...
                    {'quantity': '1/2', 'unit': 'cup', 'name': 'Parmesan cheese, shaved'},
                ...
                'instructions': [
                    {'step_number': 1, 'text': 'Wash and dry lettuce, tear into pieces.'},
                    {'step_number': 2, 'text': 'Whisk oil, lemon juice, garlic, Worcestershire, and mustard.'},
            ...
            },
            {
                ...
                'instructions': [
                    {'step_number': 1, 'text': 'Preheat oven to 350°F. Grease a 9x5 loaf pan.'},
                    {'step_number': 2, 'text': 'Mix mashed bananas with melted butter.'},
                    {'step_number': 3, 'text': 'Mix in sugar, egg, and vanilla.'},
                    {'step_number': 4, 'text': 'Sprinkle in baking soda and stir in flour.'},
                    {'step_number': 5, 'text': 'Pour into prepared pan.'},
                    {'step_number': 6, 'text': 'Bake 55-65 minutes until toothpick comes out clean.'},
                ],
